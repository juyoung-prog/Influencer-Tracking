...
- SaasDateRangeSelect: flat-SaaS 기간(시작~종료) 선택 컨트롤. 프리셋 4개(All / This week / This month / Last 30 days) + 날짜 입력 두 칸이 **같은 값**을 가리킨다 — 프리셋을 누르면 입력이 따라가고, 입력을 직접 고치면 프리셋 선택이 풀린다(맞지 않는 프리셋을 눌린 채로 두면 화면이 거짓말을 한다). 높이 36px·radius 6px로 SaasStoreSelect와 같은 컨트롤 문법. 값은 `{ from, to }` Date이고 null은 **열린 끝**(= 전체 기간)이다. 날짜 문자열 ↔ Date 변환은 schema의 `toDateKey`/`parseDateKey` 한 쌍만 쓴다 — `new Date('2026-08-20')`은 UTC 자정으로 읽혀 미국 동부에서 하루가 밀리고, 그러면 사용자가 고른 날과 세는 날이 어긋나 경계일 방문이 조용히 빠진다. 시작이 종료보다 뒤로 가면 입력을 막지 않고 **방금 건드리지 않은 쪽**을 끌고 온다. 주는 일요일 시작(매장이 미국·화면 전체가 en-US 표기). `today`는 프리셋 기준일 테스트 주입점. 입력 min/max는 두지 않는다 — 데이터 범위로 묶으면 프리셋이 그 밖의 값을 쓸 때 서로 어긋난다 (`components/templates/beautymaster/SaasDateRangeSelect.jsx`)
