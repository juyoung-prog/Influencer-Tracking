...
- SaasKpiItem(`note`): 값 아래 10px 한 줄. 수치가 스스로 말하지 못하는 단서를 붙인다(예: `Credit used 12 of 44` + `25 not recorded`). 스트립 밖 각주로 빼면 어느 셀 이야기인지 잃으므로 숫자 바로 아래에 둔다. 빈 문자열이면 렌더하지 않는다.
