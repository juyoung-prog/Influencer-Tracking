...
- InfluencerListRow: 인플루언서 리스트 행. 가로형. avatar + 이름/시간/노트 + 플랫폼·티어 + 스테이지 + overdue (`components/data-display/InfluencerListRow.jsx`)
...
- TierComparison: Tier1 vs Tier2 비교 테이블. Count 행 아래 Invited 행(초대 인원 데이터 있을 때만 표시) (`components/data-display/TierComparison.jsx`)
...
- FunnelSummaryTable: InfluencerFunnel과 같은 데이터의 표 형태 버전. Stage/Count/% of Invited 3컬럼, Invited 행 아래 Tier별 초대 인원 caption (`components/data-display/FunnelSummaryTable.jsx`)
- TierMetricsTable: Tier가 행인 통합 지표 테이블. Invited/Agreement(%)/Visited(%)/Scheduled/Content(%), Total 행은 raw count 합산 후 재계산 (`components/data-display/TierMetricsTable.jsx`)

### Section / 섹션

- DashboardHeader: 대시보드 상단 섹션. 타이틀 + SyncStatusBar + KpiBar 조합 (`components/templates/beautymaster/DashboardHeader.jsx`)
...
- AnalyticsDashboard: Analytics 탭 리포트 뷰. Funnel/Tier/Store/Category breakdown 조합, `inviteCounts` prop으로 초대 인원 데이터를 store 필터에 맞춰 반영 (`components/templates/beautymaster/AnalyticsDashboard.jsx`)
