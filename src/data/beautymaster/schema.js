 ...
 *     attended: number, goalRate: number, uploaded: number, noShow: number, scheduled: number,
...
export function deriveProgramReport(influencers, inviteCounts = {}, today = new Date(), program = INFLUENCER_PROGRAMS[0]) {
  ...
  const tierStats = tier => {
    ...
    return {
      ...
      /* 비디오 업로드 수 — 회장님 보고 항목(콘텐츠 업로드 여부). collaboShared 기준 */
      uploaded: list.filter(i => i.collaboShared).length,
